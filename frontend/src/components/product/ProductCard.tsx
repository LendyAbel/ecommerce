...
const ProductCard = ({ product }: ProductCardProps) => {
    ...
    return (
        <motion.div
            ...
            className='flex cursor-pointer flex-col overflow-hidden rounded-2xl border border-[rgba(255,255,255,0.1)] bg-[rgba(255,255,255,0.05)] backdrop-blur-sm'
            whileHover={{ y: -4, boxShadow: '0 16px 40px rgba(102,126,234,0.2)' }}
            transition={{ duration: 0.2, ease: 'easeOut' }}
        >
            <div className='h-48 w-full overflow-hidden bg-[rgba(255,255,255,0.03)]'>
                {mainImage ? (
                    <img
                        src={mainImage}
                        alt={product.name}
